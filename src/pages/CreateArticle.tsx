...
import {
  ChevronLeft, Bold, Italic, List, ListOrdered, Quote,
  Heading2, Heading3, Plus, X, Send, FileText, Info,
} from "lucide-react";

const CATEGORIES = [
  "Aquatic Ecology", "Forest Ecology", "Soil Science", "Atmospheric Science",
  "Water Quality", "Marine Biology", "Environmental Chemistry", "Climate Science",
  "Biodiversity", "Land Use & Management",
];

const KEYWORD_SUGGESTIONS = [
  "biodiversity", "climate change", "deforestation", "carbon sequestration",
  "water quality", "soil erosion", "coral reefs", "microplastics",
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div className="lg:col-span-2 space-y-4">
              ...
              >
                {keywords.length > 0 && (
                  <div className="flex flex-wrap gap-1.5 mb-2">
                    {keywords.map((k) => (
                      <span key={k} className="flex items-center gap-1 bg-green-100 text-green-700 px-2 py-0.5 rounded-full text-xs font-medium">
                        {k}
                        <button onClick={() => removeKeyword(k)} className="hover:text-green-900 transition-colors">
                          <X size={10} />
                        </button>
                      </span>
                    ))}
          ...
          <div className={`bg-white rounded-2xl border shadow-sm overflow-hidden ${errors.abstract ? "border-red-300" : "border-gray-100"}`}>
              ...
              {TOOLBAR.map((group, gi) => (
                <div key={gi} className="flex items-center gap-0.5">
                  ...
                  {group.map(({ icon: Icon, label, text, cmd, val }) => (
                    ...
                    >
                      {Icon ? <Icon size={13} /> : <span className="text-[11px] font-bold">{text}</span>}
                    </button>
                  ))}
                </div>
              ))}
            ...
            />
            {errors.abstract && (
              <p className="text-xs text-red-500 px-5 pb-3 -mt-1">{errors.abstract}</p>
            )}
          </div>
