...
import { ARTS, REVS } from "../data/mockData";
...
export default function ArticleDetail() {
    ...
    const [assignedReviewers, setAssignedReviewers] = useState<string[] | null>(null);
    const endRef = useRef<HTMLDivElement>(null);

    const a = ARTS.find((x) => x.id === id);
    const effectiveReviewers = assignedReviewers ?? a?.reviewers ?? [];

    const openAssignModal = () => {
        setSelectedReviewers(effectiveReviewers);
                                        ...
                                        <div className="flex items-center gap-2">
                                            ...
                                            <div>
                                                ...
                                                <div className="text-xs text-gray-400">
                                                    Reviewer {i + 1} · submitted 3 days ago
                                                </div>
                                            </div>
                                            <span className="ml-auto text-xs bg-blue-50 text-blue-600 px-2 py-0.5 rounded-full font-medium">
                                                Under Review
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
                                <div className="flex items-center gap-2 mb-3">
                                    <div className="w-7 h-7 bg-linear-to-br from-green-500 to-emerald-600 rounded-xl flex items-center justify-center">
                                ...
                                <div className="flex flex-wrap gap-2 mt-3">
                                    {["Rigorous methodology", "Policy relevance", "Mixed-methods approach"].map(
                                        (tag) => (
                                            <span
                                                key={tag}
                                                className="px-2.5 py-0.5 bg-green-100 text-green-700 rounded-full text-xs font-medium"
                                    ...
                                    {msgs.length === 0 && (
                                        <div className="pt-2">
                                            <p className="text-xs text-gray-400 text-center mb-3">
                                                Choose a prompt or type your own:
                                            </p>
                    ...
                    {tab === "timeline" && (
                        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
                            ...
                            <div className="relative">
                                <div className="absolute left-2.25 top-0 bottom-0 w-px bg-green-100" />
                                <div className="space-y-5">
                                    {a.timeline.map((t, i) => {
                                        ...
                                        const key =
                                            Object.keys(icons).find((k) => t.action.toLowerCase().includes(k)) ?? "";
                    ...
                    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4">
                        ...
                        <div className="space-y-0">
                            {[
                                ...
                                ["Author", a.author],
                                ["Email", a.email],
                                ["Reviewers", effectiveReviewers.length.toString()],
                            ].map(([k, v]) => (
                                <div
                                    key={k}
                    ...
                    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4">
                        ...
                        <div className="mt-3 text-xs text-gray-400 leading-relaxed">
                            {a.status === "Under Review"
                                ? `${effectiveReviewers.length} reviewer(s) assigned. Awaiting feedback.`
                    ...
                    <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg max-h-[80vh] flex flex-col">
                        ...
                        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100 shrink-0">
                            <div className="flex items-center gap-2">
                                ...
                                <h2 className="text-sm font-bold text-gray-800">Assign Reviewer</h2>
                            </div>
                            <button
                        ...
                        <div className="flex-1 overflow-y-auto p-4 space-y-2">
                            {REVS.map((rev) => {
                                const checked = selectedReviewers.includes(rev.name);
                                return (
                                    <button
                                        key={rev.id}
                                        onClick={() => toggleReviewer(rev.name)}
                                        className={`w-full text-left p-3 rounded-xl border transition-all flex items-start gap-3 ${
                                            checked
                                                ? "border-blue-300 bg-blue-50"
                                                : "border-gray-100 hover:border-gray-200 hover:bg-gray-50"
                                        } ${!rev.active ? "opacity-50" : ""}`}
                                    >
                                        <div
                                        ...
                                        <Avatar name={rev.name} sz="w-8 h-8 shrink-0" fs="text-xs" />
                                        <div className="flex-1 min-w-0">
                                            <div className="flex items-center gap-2">
                                                ...
                                                {!rev.active && (
                                                    <span className="text-xs bg-gray-100 text-gray-400 px-1.5 py-0.5 rounded-full shrink-0">
                                                        Inactive
                                                    </span>
                                                )}
                                            </div>
                                            <div className="text-xs text-gray-400 truncate">{rev.institution}</div>
                                            <div className="flex flex-wrap gap-1 mt-1">
                                                {rev.expertise.slice(0, 3).map((e) => (
                                                    <span
                                                        key={e}
                                        ...
                                        <div className="shrink-0 flex items-center gap-0.5 mt-0.5">
                                            ...
                                            <span className="text-xs font-medium text-gray-600">{rev.rating}</span>
                                        </div>
                                    </button>
                                );
                            })}
                        ...
                        <div className="px-5 py-4 border-t border-gray-100 flex items-center justify-between shrink-0">
                            ...
                            <div className="flex gap-2">
                                <button
                                    onClick={() => setShowAssignModal(false)}
                                    className="px-3 py-1.5 text-xs font-semibold text-gray-600 hover:bg-gray-100 rounded-xl transition-colors"
                                >
                                    Cancel
                                </button>
                                <button
                                    onClick={confirmAssign}
                                    className="px-4 py-1.5 bg-blue-600 hover:bg-blue-700 text-white rounded-xl text-xs font-semibold transition-colors"
                                >
                                    Confirm
                                </button>
                            </div>
